...
// Глобальное кеширование моделей и faceapi
let cachedFaceApi = null
let modelsLoaded = false
let loadingPromise = null

const loadModelsOnce = async () => {
  if (modelsLoaded) return cachedFaceApi
...
const FaceRecognition = forwardRef(
  ({ onClose, onFaceDescriptor, onCameraError }, ref) => {
    ...
    const handleVideoPlay = useCallback(async () => {
      ...
      if (!video || video.videoWidth === 0 || !faceApiRef.current) return
    ...
    const startRecognition = useCallback(async () => {
      try {
        // Сброс состояния при повторном запуске
        cleanupResources()
